...
public class RemoveAvailableShiftEventMapper implements EventMapper<RemoveAvailableShiftEvent> {

    @Override
    public RemoveAvailableShiftEvent map(final ShiftyClient client, final Packet pkt){
        final ShiftOffer shift = client.availableShifts().forId(pkt.readLong());
        if(shift == null)
            return null;
        return new RemoveAvailableShiftEvent(
                shift
        );
    }
}
